...
export const regexOnlyDigitsWhiteSpacePunctuation =
  /^[\d\s!"#$%&'()*+,\-./:;<=>?@[\]^_`{|}~]*$/;

export class CustomTreeWalker {
  options: DropdownOptions;
  customFilter: { acceptNode(node: Node): number };
    ...
    this.customFilter = {
      ...
      acceptNode(node: Node): number {
        ...
        if (
          anyParentSatisfies(
            ...
            (e) =>
              ...
              e.className.includes("skipTranslate")
          )
        ) {
        ...
        if (
          anyParentSatisfies(
            node,
            (e) => e.getAttribute("data-rosetta-skipTranslate") === "true"
          )
        ) {
          return NodeFilter.FILTER_REJECT;
        }
        ...
        for (let selector of options.ignoreSelectors) {
          if (
            anyParentSatisfies(
              node,
              (e) => e instanceof Element && e.matches(selector)
            )
          ) {
            return NodeFilter.FILTER_REJECT;
          }
        ...
        if (
          node.parentNode &&
          forbiddenTags.includes(node.parentNode.nodeName.toLocaleLowerCase())
        ) {
          return NodeFilter.FILTER_REJECT;
        }
 ...
 */
export function anyParentSatisfies(
  node: Node,
